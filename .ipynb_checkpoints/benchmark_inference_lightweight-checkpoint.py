...
from sklearn.metrics import accuracy_score, f1_score, precision_recall_fscore_support  # <-- 添加导入
...
class FinalLightweightModel(nn.Module):
    ...
    def forward(self, content_inputs, pixel_values, graph_batch):
        ...
        print(f"Content 模块时间: {content_time:.4f} 秒")
        print(f"Vision 模块时间: {vision_time:.4f} 秒")
        print(f"GAT 模块时间: {gat_time:.4f} 秒")
