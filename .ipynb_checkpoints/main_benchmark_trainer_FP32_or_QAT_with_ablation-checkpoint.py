...
class AttentionQuantWrapper(nn.Module):
    def __init__(self,
                 module): super().__init__(); self.dequant = DeQuantStub(); self.attention = module; self.quant = QuantStub()
...
class MultiModalPromptModel(nn.Module):
    def __init__(self, plm, template: Template, verbalizer: ManualVerbalizer):
        ...
        clip_feature_size = 512  # CLIP-base 特征维度
...
def preprocess_and_tokenize_dataset(dataset: List[InputExample], template: Template, tokenizer: PreTrainedTokenizer,
                                    wrapper_class, config: object, default_image_features: torch.Tensor) -> List[
    InputFeatures]:
    ...
    for example in tqdm(dataset, desc="Tokenizing Dataset"):
        ...
        if 'image_features' in signature(InputFeatures.__init__).args and hasattr(config,
                                                                                  'use_images') and config.use_images:
...
def create_prompt_model(config: object, scripts_path: str, device: str) -> Tuple:
    """创建模型实例，并根据配置决定是否为QAT植入wrappers。"""
    ...
    # 仅在需要进行全模块量化时才植入wrappers
    if config.qat_ablation_type in ["per_channel_full", "per_tensor_full"]:
        logger.info(f"--> 为 QAT 策略 '{config.qat_ablation_type}' 准备模型骨架 (植入所有Wrappers)...")
...
def prepare_model_for_qat(model, experiment_type: str):
    """根据指定的消融实验类型，为模型准备QAT。"""
    if experiment_type == "none":
        return  # FP32 模式不执行任何操作

    qconfig = None
    if experiment_type == "per_tensor_linear_only":
        logger.info("--> [QAT] 准备: Per-Tensor + 仅线性层量化...")
        ...
        logger.info("--> [QAT] 准备: Per-Channel + 全模块量化...")
        from torch.ao.quantization.observer import PerChannelMinMaxObserver
        base_qconfig = torch.ao.quantization.get_default_qat_qconfig('fbgemm')
        per_channel_weight_observer = PerChannelMinMaxObserver.with_args(dtype=torch.qint8,
        ...
        if not any(isinstance(m, BertEncoderQuantWrapper) for m in model.modules()):
            raise RuntimeError("Per-Channel Full量化需要先植入QuantWrappers。请检查create_prompt_model逻辑。")
        # 遍历所有模块并应用qconfig
        ...
        logger.info("--> [QAT] 准备: Per-Tensor + 全模块量化 (默认策略)...")
        qconfig = torch.ao.quantization.get_default_qat_qconfig('fbgemm')
        # 确认wrappers已存在
        if not any(isinstance(m, BertEncoderQuantWrapper) for m in model.modules()):
            raise RuntimeError("Per-Tensor Full量化需要先植入QuantWrappers。请检查create_prompt_model逻辑。")
        # 遍历所有模块并应用qconfig
        modules_to_skip = [m.attention for m in model.modules() if isinstance(m, AttentionQuantWrapper)]
        for name, module in model.named_modules():
            if any(module in parent.modules() for parent in modules_to_skip): continue
        ...
        raise ValueError(f"未知的QAT实验类型: {experiment_type}")

    model.train()
    torch.quantization.prepare_qat(model, inplace=True)
    logger.info("--> QAT 模型已为当前实验准备好。")
...
def measure_full_performance(model, test_dataset, config, device):
    ...
    accuracy_dataloader = DataLoader(test_dataset, batch_size=config.batch_size, collate_fn=InputFeatures.collate_fct,
                                     shuffle=False)
    all_preds, all_labels = [], []
    with torch.no_grad():
    ...
    accuracy_metrics = classification_report(all_labels, all_preds, output_dict=True, zero_division=0, digits=4,
                                             target_names=model.verbalizer.classes)
    ...
    def get_latency_stats(latencies_ms: List[float]) -> Dict:
        ...
        return {'avg': np.mean(arr), 'p50': np.percentile(arr, 50), 'p90': np.percentile(arr, 90),
...
def save_model_and_report(final_model, accuracy_metrics, performance_metrics, config, start_time, fold_results=None,
                          mean_acc=0, std_acc=0, best_val_acc=0):
    ...
    if ablation_type == "none":
        ...
        model_type_str = f"-QAT_{ablation_type}"

    if hasattr(config, 'use_images') and config.use_images:
        model_type_str += "_MultiModal"
    ...
    if config.use_k_fold and fold_results is not None:
        report_content.extend(["\n" + "-" * 15 + " 2. K-折交叉验证结果 " + "-" * 15,
                               f"各折最佳验证准确率: {[round(r, 4) for r in fold_results]}",
                               f"平均验证准确率: {mean_acc:.4f} (±{std_acc:.4f})"])
    else:
        report_content.extend(["\n" + "-" * 15 + " 2. 验证集表现 " + "-" * 15, f"最佳验证准确率: {best_val_acc:.4f}"])

    report_content.extend(["\n" + "-" * 15 + " 3. 最终测试集精度指标 " + "-" * 15,
                           f"整体准确率: {accuracy_metrics['overall_accuracy']:.4f}"])
    ...
    for class_name in class_names_from_report:
        if class_name in accuracy_metrics:
            ...
            report_content.append(
                f"{class_name:<15} {metrics['precision']:.4f}     {metrics['recall']:.4f}     {metrics['f1-score']:.4f}")
    report_content.append("-" * 50)
    macro_avg = accuracy_metrics['macro avg']
    report_content.append(
        f"{'Macro Avg':<15} {macro_avg['precision']:.4f}     {macro_avg['recall']:.4f}     {macro_avg['f1-score']:.4f}")

    report_content.extend(["\n" + "-" * 15 + " 4. 最终模型性能指标 " + "-" * 15, f"评测设备: {final_model.device}",
                           f"模型磁盘大小: {model_stats['size_mb']:.2f} MB",
                           f"参数量: {model_stats['num_params'] / 1e6:.2f} M",
                           f"吞吐量: {performance_metrics['throughput_sps']:.2f} samples/s (在 batch_size={config.batch_size} 下测得)"])
    ...
    report_content.append(
        f"{'End-to-End':<18} | {e2e['avg']:<10.2f} | {e2e['p50']:<10.2f} | {e2e['p90']:<10.2f} | {e2e['p95']:<10.2f} | {e2e['p99']:<10.2f}")
    report_content.append(
        f"{'Model Forward':<18} | {fwd['avg']:<10.2f} | {fwd['p50']:<10.2f} | {fwd['p90']:<10.2f} | {fwd['p95']:<10.2f} | {fwd['p99']:<10.2f}")
    report_content.append("=" * 80)
...
def run_k_fold_training(config, dataset, scripts_path, device, default_image_features):
    ...
    train_val_pool, final_test_set = train_test_split(full_dataset, test_size=config.k_fold_test_split_size,
                                                      random_state=config.seed, stratify=labels)
    skf = StratifiedKFold(n_splits=config.k_folds, shuffle=True, random_state=config.seed)
    fold_results, train_val_pool_np, train_val_pool_labels = [], np.array(train_val_pool), [ex.label for ex in
                                                                                            train_val_pool]
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_val_pool_np, train_val_pool_labels)):
        ...
        tokenized_val_fold = preprocess_and_tokenize_dataset(val_data_fold, mytemplate, tokenizer, WrapperClass, config,
        ...
        for epoch in range(config.max_epochs):
            prompt_model.train()
            train_dataloader_fold = DataLoader(tokenized_train_fold, sampler=RandomSampler(tokenized_train_fold),
    ...
    tokenized_train_val_pool = preprocess_and_tokenize_dataset(train_val_pool, mytemplate, tokenizer, WrapperClass,
                                                               config, default_image_features)
    final_train_dataloader = DataLoader(tokenized_train_val_pool, sampler=RandomSampler(tokenized_train_val_pool),
                                        batch_size=config.batch_size, collate_fn=InputFeatures.collate_fct)
...
def run_single_shot_training(config, dataset, scripts_path, device, default_image_features):
    ...
    sampler = FewShotSampler(num_examples_per_label=config.shot, also_sample_dev=True,
                             num_examples_per_label_dev=config.shot)
    ...
    for epoch in range(config.max_epochs):
        prompt_model.train()
        train_dataloader = DataLoader(tokenized_train_set, sampler=RandomSampler(tokenized_train_set),
                                      batch_size=config.batch_size, collate_fn=InputFeatures.collate_fct)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="统一训练与评估脚本 (配置驱动，整合QAT消融实验)")
    parser.add_argument("--config_file", type=str, default="./config/config_main.txt", help="配置文件路径")
    ...
    if not hasattr(config, 'use_qat') or not config.use_qat:
        ...
        logger.warning(
            "Config 'use_qat' is true, but 'qat_ablation_type' is not defined. Defaulting to 'per_tensor_full' to match baseline behavior.")
        config.qat_ablation_type = "per_tensor_full"

    is_qat = config.qat_ablation_type != "none"

    if torch.cuda.is_available():
        device = "cuda"; logger.info(f"--> 检测到CUDA设备，将在GPU上进行训练。")
    else:
        device = "cpu"; logger.info(f"--> 未检测到CUDA设备，将在CPU上进行训练和评估。")

    if is_qat:
        torch.backends.quantized.engine = "fbgemm"
        logger.info(f"--> QAT 已启用 (策略: {config.qat_ablation_type})，设置量化后端引擎为 'fbgemm'。")
    else:
        logger.info("--> 本次运行为标准 FP32 训练。")
        ...
        try:
            ...
            for example in tqdm(all_examples, desc="提取所有图像特征"):
                ...
                if isinstance(example.meta, str):
                    img_path_str = example.meta; example.meta = {}
                elif isinstance(example.meta, dict):
                    img_path_str = example.meta.get('img_path')
    ...
    if config.use_k_fold:
        final_model, test_dataset, fold_results, mean_acc, std_acc = run_k_fold_training(config, dataset, scripts_path,
                                                                                         device, default_image_features)
    else:
        final_model, test_dataset, best_val_acc = run_single_shot_training(config, dataset, scripts_path, device,
                                                                           default_image_features)
