...
# - 将训练、详细评估和保存流程合并到一个脚本中，杜绝因保存/加载导致的环境不一致问题。
# - 在训练结束后，使用内存中的最终模型对象直接进行精度和性能测试。
# - 新增 MultiModalPromptModel 以正确融合图像与文本特征。
# - 包含了完整的Wrapper定义、模型创建、训练逻辑、性能评测、报告生成以及【完整的图像处理逻辑】。
# - 本脚本未经任何简化或省略。
...
class MultiModalPromptModel(nn.Module):
    def __init__(self, plm, template: Template, verbalizer: ManualVerbalizer):
        super().__init__()
        # 内部的PromptForClassification只负责处理文本
        ...
        # 将verbalizer暴露给顶层，方便报告生成等后续操作
        self.verbalizer = self.prompt_model.verbalizer
        # 通过property代理device属性，确保总能获取到正确的设备信息
        # self.device = self.prompt_model.device
    ...
    def forward(self, inputs: InputFeatures) -> torch.Tensor:
        # 1. 使用内部的 prompt_model 获取文本部分的 logits
...
#                                 QAT 消融实验准备函数 (终极兼容版)
# ==================================================================================================
def prepare_model_for_qat_ablation(model, experiment_type: str):
    """根据指定的消融实验类型，为模型准备QAT (使用最稳健、最兼容的API修改方式)。"""
    qconfig = None
    if experiment_type == "per_tensor_linear_only":
        logger.info("--> [消融实验] 准备: Per-Tensor + 仅线性层量化...")
        ...
        logger.info("--> [消融实验] 准备: Per-Channel + 全模块量化...")
        ...
        if not any(isinstance(m, BertEncoderQuantWrapper) for m in model.modules()):
            raise RuntimeError("Per-Channel Full量化需要先植入QuantWrappers。")
        ...
        logger.info("--> [消融实验] 准备: Per-Tensor + 全模块量化 (原版默认策略)...")
        qconfig = torch.ao.quantization.get_default_qat_qconfig('fbgemm')
        if not any(isinstance(m, BertEncoderQuantWrapper) for m in model.modules()):
            raise RuntimeError("Per-Tensor Full量化需要先植入QuantWrappers。")
        modules_to_skip = [m.attention for m in model.modules() if isinstance(m, AttentionQuantWrapper)]
        for name, module in model.named_modules():
            if any(module in parent.modules() for parent in modules_to_skip): continue
            if 'embeddings' in name and isinstance(module, nn.LayerNorm): continue
        ...
        raise ValueError(f"未知的消融实验类型: {experiment_type}")
    model.train()
...
def save_model_and_report(final_model, accuracy_metrics, performance_metrics, config, start_time, fold_results=None, mean_acc=0, std_acc=0, best_val_acc=0):
    ...
    ablation_type = config.ablation_experiment
...
def run_k_fold_training(config, dataset, scripts_path, device, default_image_features):
    ...
    is_qat = config.ablation_experiment != "none"

    for fold, (train_idx, val_idx) in enumerate(skf.split(train_val_pool_np, train_val_pool_labels)):
        logger.info(f"\n========== 开始第 {fold + 1}/{config.k_folds} 折 ==========")
        needs_wrappers = config.ablation_experiment in ["per_channel_full", "per_tensor_full"]
        prompt_model, tokenizer, mytemplate, WrapperClass = create_prompt_model(config, scripts_path, device, quantize=needs_wrappers)
        if is_qat:
            prepare_model_for_qat_ablation(prompt_model, config.ablation_experiment)
    ...
    needs_wrappers = config.ablation_experiment in ["per_channel_full", "per_tensor_full"]
    final_model, tokenizer, mytemplate, WrapperClass = create_prompt_model(config, scripts_path, device, quantize=needs_wrappers)
    if is_qat:
        prepare_model_for_qat_ablation(final_model, config.ablation_experiment)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="统一训练与评估脚本 (含QAT消融实验)")
    ...
    parser.add_argument("--ablation_experiment", type=str, default="none", 
                        choices=["none", "per_tensor_linear_only", "per_channel_full", "per_tensor_full"],
                        help="选择要运行的QAT消融实验类型。'none'表示FP32, 'per_tensor_full'是原版默认策略。")
    args = parser.parse_args()
    config = load_config_from_file(args.config_file)
    set_seed(config.seed)

    config.ablation_experiment = args.ablation_experiment
    is_qat = config.ablation_experiment != "none"
    ...
    if is_qat:
        ...
        logger.info(f"--> QAT 已启用 ({config.ablation_experiment})，设置量化后端引擎为 'fbgemm'。")
    else:
        logger.info("--> 本次运行为标准FP32训练。")
